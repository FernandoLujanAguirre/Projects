...
#v3 = [-c4*sin(u[4])
#       -c5*sin(u[5])-b5*du[5]];
#τ = M33bar*v3 + h3bar #No cancelar a la friccion para que las velocidades no se vayan al inf
#
##Sistema #v3
#du[1:5] .= (u[6:10] )
...
#b3 = 1E-6;
#c3 = 1.0E-7;
#v2 = -c3*sin(u[3])-b3*du[3];
#v3 = -pinv(M23bar)*only(inv(M22)*v2 + only(h2bar));
#τ = M33bar*v3 + h3bar
##Sistema v2
#du[1:5] .= (u[6:10] )
#du[6:7] .= -inv(M11)*(M13bar*v3 + h1bar);
...
#      -du[2]];
#
#v1 = Kd*(ep) + Kp*e; #control x y y
v1 = [0;
      0]
#v2 =  0.4*(-du[3]) + 0.1*(-u[3]) #controla #z

#v3 = -inv(M13bar)*(M11*v1 + h1bar)#para controlar a x y y
#v3 = -pinv(M23bar)*only(inv(M22)*v2 + only(h2bar));#para controlar a z
Kd = [1E1 0
      0 1E-3];#-3 WF
Kp = [1E1 0
      0 1E-3];
...
#TRBDF2
global sol = solve(prob,TRBDF2(),maxiters=2_000_000,tstops=savetimes)
...
#Posiciones
traces1 = PlotlyJS.scatter(x=tiempo, y=t1, mode="lines",name="t1(t)",line=attr(color="red"))
traces2 = PlotlyJS.scatter(x=tiempo, y=t2, mode="lines",name="t2(t)",line=attr(color="green"))
traces3 = PlotlyJS.scatter(x=tiempo, y=t3, mode="lines",name="t3(t)",line=attr(color="blue"))
