...
function rotor(du,u,p,t)
 ...
 C=[c11 c12 c13 c14 c15
    ...
    g = inv(M)
    # VISCOSIDAD
    b₄ = 0.0 #Friccion en y   0.001
    b₅ = 0.0  #Friccion en x   0.0001
    v₄ = b₄*du[4]
    v₅ = b₅*du[5]
    V = [0;0;0;v₄;v₅]
    f = g * (-C * du[1:5] )
    ...
    #Gimbal Pos = u[4] , Vel = du[4], Ac = du[9]
    e1vel2 = du[4] - 10.0;
    ep1vel2 = du[9];
    k₀vel2 = 0.0E-1 #
    k₁vel2 = 2*sqrt(k₀vel2)   ;
    u4 = - (k₀vel2*e1vel2 + k₁vel2*ep1vel2);#y
    
    #5Prisma Pos = u[5] , Vel = du[5], Ac = du[10]
    e1vel = du[5];
    ep1vel = du[10];
    k₀vel = 2.1E-1; #
    k₁vel = 2*sqrt(k₀vel)   ;
    u5 = - (k₀vel*e1vel + k₁vel*ep1vel); #x
    ...
    τ = (inv(Bᵥ) *  (vᵥ - Aᵥ)) + (inv(Bₚ) *  (vₚ - Aₚ))
    
    
    du[1:5] .= u[6:10]     
    du[6:10] .= f + g* [0;0;0;τ[1];τ[2]] 
   end
...
#TRBDF2
global sol = solve(prob,Tsit5(),tstops=savetimes)
...
pos = plot([traces1,traces2,traces4,traces5], Layout(title="Posiciones"))
vel = plot([traces6,traces7,traces9,traces10], Layout(title="Velocidades"))
